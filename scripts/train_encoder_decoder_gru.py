...
def prepareData(filepath, lang1, lang2, max_size, reverse=False):
    input_vocab, output_vocab, pairs = create_vocabs(filepath, lang1, lang2, max_size = max_size, reverse = reverse)
    print(f"Found {len(pairs)} sentence pairs")
    print("Add sentences to vocab")
    for pair in pairs:
        input_vocab.add_sentence("".join(pair[0]))
        output_vocab.add_sentence("".join(pair[1]))
    print("Number of words:")
    print(input_vocab.lang_code, input_vocab.n_words)
    print(output_vocab.lang_code, output_vocab.n_words)
    return input_vocab, output_vocab, pairs
...
def evaluate(encoder, decoder, sentence, src_vcb, tgt_vcb, device):
    with torch.no_grad():
        ...
        encoder_outputs, encoder_hidden = encoder(input_tensor)
        decoder_outputs, _, decoder_attention = decoder(encoder_outputs, encoder_hidden, None)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_checkpoint", default = "./state_0000000100_gru.pt", h                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               elp = "Model checkpoint")
    ...
    parser.add_argument("--score_graphs", default = ["dev_scores.png", "test_scores.png", "train_scores.png"], nargs = 3, help = "")
    ...
    parser.add_argument("--print_every", default = 10, type = int, help = "Print every epochs")
    parser.add_argument("--plot_every", default = 10, type = int, help = "Every n iterations, add a point for plotting")
    parser.add_argument("--check_every", default = 10, type = int, help = "Check every epochs")
    parser.add_argument("--test_every", default = 10, type = int, help = "How often to run dev and test sets")
    ...
    dev_set = get_pairs(args.dev_file, max_size = 5000)
    test_set = get_pairs(args.test_file, max_size = 5000)
    train_eval_set = get_pairs(args.train_file, max_size = 5000)
    ...
    print(f"GRAPHS: {args.score_graphs}")
    if args.load_checkpoint:
        state = torch.load(args.load_checkpoint)
        epoch_iter = state["epoch_num"]
        print(f"Starting training at iter: {epoch_iter}")
        # src_vocab = Vocab(state["src_vocab"])
        # tgt_vocab = Vocab(state["tgt_vocab"])
        plot_losses = state["train_losses"]
        dev_scores = state["dev_bleu_scores"]
        test_scores = state["test_bleu_scores"]
        train_scores = state["train_scores"]


    while epoch_iter < args.epochs:
        epoch_iter += 1
        ...
        if epoch_iter % args.print_every == 0:
            print_loss_avg = print_loss_total / args.print_every
            print_loss_total = 0
            print(f'{timeSince(start, epoch_iter / args.epochs)} ({epoch_iter} {epoch_iter / args.epochs * 100:.0f}%) {print_loss_avg:.4f}')

        if epoch_iter % args.plot_every == 0:
            plot_loss_avg = plot_loss_total / args.plot_every
            plot_losses.append(plot_loss_avg)
            plot_loss_total = 0

        if epoch_iter % args.test_every == 0:
            dev_score = evaluate_on_set(encoder, decoder, dev_set, src_vocab, tgt_vocab)
            test_score = evaluate_on_set(encoder, decoder, test_set, src_vocab, tgt_vocab)
            train_eval_score = evaluate_on_set(encoder, decoder, train_eval_set, src_vocab, tgt_vocab)
            dev_scores.append(dev_score)
            test_scores.append(test_score)
            train_scores.append(train_eval_score)
            print(f"Dev score for epoch {epoch_iter} is {dev_score}. Test score is {test_score}")
            state = {"epoch_num": epoch_iter,
                "encoder_model_state": encoder.state_dict(),
                "decoder_model_state": decoder.state_dict(),
                ...
                "train_losses" : plot_losses,
                "train_scores": train_scores,
                "dev_bleu_scores" : dev_scores,
                "test_bleu_scores" : test_scores
                }
            filename = f'state_{epoch_iter:010d}.pt'
            torch.save(state, filename)
            print(f"wrote checkpoint to {filename}")

    print(plot_losses)
    print(dev_scores)
    print(test_scores)
    plot_data(plot_losses, args.plot_every, "Epochs", "Train Loss", "Train Loss vs Epochs", args.loss_graphs[0])
    plot_data(dev_scores, args.test_every, "Epochs", "Dev Bleu Score", "Dev Bleu Score vs Epochs", args.score_graphs[0])
    plot_data(test_scores, args.test_every, "Epochs", "Test Bleu Score", "Test Bleu Score vs Epochs", args.score_graphs[1])
    plot_data(train_scores, args.test_every, "Epochs", "Train Bleu Scores", "Train Bleu Score vs Epochs", args.score_graphs[2])
